...
try{
  ...
  if(publicUser && inspType != 'Final Landscape Design' || 'Rough Fire Sprinkler' || 'Fire High Fire Landscaping' || 'Fire Final') {
  ...
  } else if(inspType != 'Final Landscape Design' || 'Rough Fire Sprinkler' || 'Fire High Fire Landscaping' || 'Fire Final') { //If scheduled via Accela Mobile app, assign to current user
...
try{
  ...
  if(inspType == 'Final Landscape Design'){ //If Final Landscape Design, assign to Jasmine Showers
    ...
    logDebug("Final Landscape Design Inspection Scheduled: " + inspectorAssign);
  }
