/*------------------------------------------------------------------------------------------------------/
| Program : ACA_BLD_OTC_SOLAR_PV_ONLOAD_PAGEFLOW.js
| Event   : ACA_BLD_OTC_SOLAR_PV_ONLOAD_PAGEFLOW Event
|
...
try {
   var hide = true;
    if (( String(AInfo["Solar Photovoltaic"]).toLowerCase() == 'checked' ) 
    || ( String(AInfo["Solar Photovoltaic and Battery Energy"]).toLowerCase() == 'checked' )
     ) {
        hide = false;
        for(var x in AInfo){
            logDebug(x + " + " + AInfo[x]);
    ...
    showDebug = true;
    logDebug("An error has occurred in ACA_BLD_OTC_SOLAR_PV_ONLOAD_PAGEFLOW: Main function: " + err.message);
    logDebug(err.stack);
}
