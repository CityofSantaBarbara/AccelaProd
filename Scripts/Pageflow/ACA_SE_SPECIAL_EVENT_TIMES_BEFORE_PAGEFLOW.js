/*------------------------------------------------------------------------------------------------------/
| Program : ACA_SE_SPECIAL_EVENT_TIMES_BEFORE_PAGEFLOW.js
| Event   : ACA_SE_SPECIAL_EVENT_TIMES_BEFORE_PAGEFLOW Event
|
...
try {
    var rules = [];
    rules["SpecialEvent/Special Event/NA/Application"] = ["Setup", "Event", "Breakdown"];

    var myRules = rules[appTypeString];
    if (myRules) {
        var passedRules = true;
        for (var i in myRules) {
            var thisRule = myRules[i];
            if (typeof(EVENTINFO) == "object") {
                var foundThisRule = false;
                for (var j in EVENTINFO) {
                    if (thisRule == EVENTINFO[j]["Event Type"]) {
                        foundThisRule = true;
                    }
                }
                if (!foundThisRule) {
        ...
        if (!passedRules) {
            showMessage = true;
            comment("You must add a minimum of 1 row for Setup, Event, and Breakdown. Please add multiple rows if more that one is needed for each.: " + myRules.join(", "));
            cancel = true;
        }
    }
} catch (err) {
